 ...
 * @param  {any...}            args      arguments that with pass to generator
 * @return {Promise}                     es6 promise object
 * @static
 * @private
 */
exports.wrapToPromise = function wrapToPromise(gen, self, Promise) {
  var defered = {};
  ...
  function next(inst, result, resultPromise) {
    var isPromise = 
        'object' === typeof result.value && 
        'function' === typeof result.value.then;
    if (result.done) {
      return resultPromise.resolve();
    }
    if (isPromise) {
      // it is a promise
      result.value.then(function (value) {
        wrapThrow(function () {
          next(inst, inst.next(value), resultPromise);
        });
      }, function (err) {
        wrapThrow(function () {
          next(inst, inst.throw(err), resultPromise);
        });
      })
    } else {
      wrapThrow(function () {
        next(inst, inst.next(result.value), resultPromise);
      });
    }
  }

...
;var __append = function (data, transforms) {
  ...
  if (data != null && "function" === typeof data.then) {
    ...
    if (data.outputStream) {
      ...
      data.then(function () {
        // do not resolve here, since data haven't got flushed
        // defered.resolve('');
      }, function (err) {
        defered.reject(err);
      });
      data.outputStream.pipe(__outputStream, {end: false})
      data.outputStream.on('end', function () {
        // console.log('[ continue evalutae ]' +  __filename + ' : ' + __line)
        data.outputStream.unpipe(__outputStream)
        defered.resolve('');
      })
    } else {
...
return p;
    ...
    var returnedFn = function (data) {
      ...
      Object.defineProperty(outputStream, 'shouldWaitFlush', {
        get: function () {return p.shouldWaitFlush},
        set: function (val) {p.shouldWaitFlush = val}
      })

...
;var __append = function (data, transforms) {
  ...
  if (data != null && "function" === typeof data.then) {
    ...
    if (data.outputStream) {
      data.then(function () {
        // console.log('[ waiting for stream end to continue evalutae ]' +  __filename + ' : ' + __line)
        // defered.resolve('');
      }, function (err) {
        defered.reject(err);
      })
      data.outputStream.on('data', function (data) {
        // console.log('[ forward data ] ' +  __filename + ' : ' + __line + ' : ' + JSON.stringify(data.toString()))
        data = data.toString();
        __outputStream.write(transformFunc(data));
      })
      data.outputStream.on('end', function () {
        // console.log('[ continue evalutae ]' +  __filename + ' : ' + __line)
        defered.resolve('');
      })
    } else {
      data.then(function (data) {
        // console.log('[ continue evalutae ] ' +  __filename + ' : ' + __line + ' : ' + data.toString())
        data = data.toString();
        __outputStream.write(transformFunc(data));
    ...
    // console.log('[ halt evalutae]  in ' +  __filename + ' : ' + __line)
    // console.log(defered.promise);
    return defered.promise;
  } else if (data != null) {
    // console.log('[ write data ] ' +  __filename + ' : ' + __line + ' : ' + JSON.stringify(data.toString()))
    data = data.toString();
    return __outputStream.write(transformFunc(data));
  }
};
\n`;
      if (opts._with !== false) {
    ...
    try {
      fn = new GeneratorFunction(opts.localsName + ', escape, include, rethrow, __Promise, __wrapToPromise, __outputStream', src);
      // console.log(fn)
    }
    catch(e) {
    ...
    if (opts.client) {
      var wrappedFn = new Function('local, escape, include, rethrow, Promise, __Stream',
      `
var __Stream = __Stream || Stream;
var wrapToPromise = ${utils.wrapToPromise.toString()};
var outputStream = new Stream.PassThrough;
var p = utils.wrapToPromise(${fn.toString()}, this, Promise, local, escape, include, rethrow, Promise, wrapToPromise, outputStream, Transform);
...
return p;
    ...
    var returnedFn = function (data) {
      ...
      var outputStreamBuffer = new PassThrough;
      outputStream.pipe(outputStreamBuffer, {end: false});
      
      var p = utils.wrapToPromise(fn, opts.context, Promise, data || {}, escape, include, rethrow, Promise, utils.wrapToPromise, outputStream);
      ...
      // hook for buffer output
      p.defered.onResolve = function (defered) {
        // console.log('\n[ templete end ' + opts.filename + ' ]\n' + p.outputBuffer + '\n[/templete end]\n')
        outputStream.end();
        outputStreamBuffer.end();
        defered.resolve(p.outputBuffer)
      }
      // hook for buffer output
        ...
        if ((include = line.match(/^\s*include\s+(\S+)/))) {
          ...
          if (opening && (opening == '<' + d || opening == '<' + d + '-' || opening == '<' + d + '_')) {
            ...
            if (self.opts.compileDebug) {
              includeSrc =
                  '    ; yield __wrapToPromise(function* (){' + '\n'
                  + '      var __line = 1' + '\n'
                  ...
                  + '        rethrow(e, __lines, __filename, __line);' + '\n'
                  + '      }' + '\n'
                  + '    ; }, this, __Promise).execute()' + '\n';
            }else{
              includeSrc = '    ; yield __wrapToPromise(function* (){' + '\n' + includeObj.source +
                  '    ; }, this, __Promise).execute()' + '\n';
            }
